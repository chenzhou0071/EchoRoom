...
| 账号格式 | `[A-Za-z0-9][A-Za-z0-9_]{2,19}`（3-20 位、不能以 `_` 开头），原样存储、大小写敏感（`Alice` 与 `alice` 为两个独立账号） | 用户确认：不同大小写视为不同账号、不能以 `_` 开头；密码 6-64 字符、仅英文/数字/符号（不能含中文、空格）、大小写敏感 |
...
- 注册/资料：`邀请码错误` / `账号已存在` / `账号格式不合法（3-20 位字母数字下划线，不能以 _ 开头）` / `密码至少 6 位` / `密码只能包含英文、数字或符号` / `昵称不合法（1-24 字符）` / `未开放注册` / `头像过大（上限 64KB）`
...
- **改资料入口**：C 阶段不做——资料弹窗仅在注册成功后弹出一次；改资料入口随 D 设置系统一并提供
...
- 登出/切换账号 UI、改资料入口（点击自己名字重开资料弹窗）——统一由 D 设置系统提供；登录失效自动回登录页兜底
- 改密码 / 找回密码 / 邮箱验证
- 登录限速与防爆破、验证码
