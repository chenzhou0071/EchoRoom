...
> ⚠ 执行记录（T10 落地与 R2）：点投屏/开摄像头成功后**自动进入自预览**（`videoView.preview(kind, getStream(kind))`），不自动弹面板（Step 2(b) 相应调整）；面板由预览视图右下角 ⚙（#view-settings）或投屏中的卡片按钮开关；sharePop 增加"正在观看：…"行（R1：`updateShareViewers(uids)`，随 viewer_count 更新、打开面板时渲染）；`viewer_count`/`request_keyframe` 监听已在 T9 加入（Step 4(d) 实际只插入 `stream_state`/`share_audio`）。
...
单客户端（1–6）+ 双客户端（7–9；R2：发送端入口 = 卡片按钮起流 → 自动进自预览，面板入口 = 预览视图右下角 ⚙）：

1. `cargo tauri dev` + 本地服务器；连接后自己卡片：投屏/摄像头按钮可点（不再是"即将推出"禁用态）
2. 点投屏 → 系统选择器选屏 → 按钮变蓝并自动进入自预览（全屏视图显示屏幕画面，右下角出现 ⚙）
3. 预览中点 ⚙ → 投屏设置面板开（出现在按钮上方）；再点 ⚙ 或点画面其他区域 → 关；再点 ⚙ → 重开（预览中 #view 覆盖卡片，面板入口以 ⚙ 为准）
4. 面板切"1080p15" → devtools 无报错；勾/取消"共享系统声音" → Rust stdout 出现"[screen] 屏幕声音采集启动/停止"
5. 点摄像头 → 按钮变蓝并自动进入自预览（投屏中则显示为右下角小窗、可拖拽/缩放）；再点 → 复原（该路预览同步退出；两路全停视图退出）
6. 面板"停止投屏" → 按钮复原、预览退出；或从系统共享条"停止共享" → 同样复原且面板自动关闭
7. 双客户端：对方卡片出现"投屏中"角标 + 纱（中央播放按钮）；点击进入观看（徽标"N 人在看"）；再点纱退出观看；流主面板"正在观看"行列昵称
...
### R5（2026-09-20）：投屏者自卡薄纱入口（「←返回」后可回到自预览）

**需求（用户实测确认）**：投屏者点预览左上「←返回」退出后，无入口回到自预览；自己卡片应像他人一样显示薄纱+播放按钮，点击即回到自预览视图。

**实现**：
- `app.js` `buildCard`：薄纱渲染条件 `!isSelf && m.streams` → `m.streams`（自己/他人共用）；点击分支：预览中 → `end()`，否则按本地活跃流逐路 `videoView.preview(kind, getStream(kind))`（投屏与摄像头在则全带）；角标仍仅他人显示，自己状态由卡片按钮颜色体现。
- 流状态来源不变：服务器 `StreamState` 本就广播回所有人（含自己），自分支数据可靠。

**验收增量（Task 11 验收表追加；22）**：

| # | 操作 | 预期 |
|---|------|------|
| 22 | 投屏（可带摄像头）中点预览左上「←返回」 | 回房间视图，自己卡片出现薄纱+播放按钮；点薄纱回到自预览（双路齐全）；全部停止后薄纱消失 |

**执行时机**：2026-09-20 当日实现并构建（安装包 0.1.3）。

### R6（2026-09-20）：观看端「画面全屏」（替代窗口全屏）

**需求（用户澄清）**：⛶ 要的是投屏**画面**全屏，不是应用窗口全屏——观看时点 ⛶ 后画面铺满整个屏幕，房间 UI 全部被盖住。

**实现**：
- `style.css`：`.view.view-full`（position: fixed; inset: 0; z-index: 25——高于房间 UI、低于弹层 30）+ 主画面 `width/height: 100% + object-fit: contain`（等比放大、可留黑边，窗口缩放自动适配）。
- `video_view.js`：`toggleFullscreen` 重写为「`view-full` 类 + 窗口 setFullscreen」组合；`exitViewFull()` 统一复位并接入 `end()`（×关视图/对方停播/切流等所有退出路径窗口自动还原）；ESC 退出；预览模式仍无 ⛶（仅观看端）。

**验收增量（Task 11 验收表追加；23）**：

| # | 操作 | 预期 |
|---|------|------|
| 23 | 观看中点 ⛶（或 ESC 退出） | 画面铺满屏幕（纯黑底、等比居中）；全屏中点 × 或对方停播 → 视图退出且窗口自动复原；全屏下 🔊 弹层浮于画面之上 |

**执行时机**：2026-09-20 当日实现并构建（安装包 0.1.4）。

### R7（2026-09-20）：投屏设置面板只朝下（不再翻到按钮上方）

**需求（用户实测确认）**：预览视图右下 ⚙ 打开的面板在下方空间不足时会翻到按钮上方、遮住画面；要求只朝下展开。

**实现**：
- `app.js` `openSharePop`：删除「放不下 → 翻到按钮上方」，改为下缘超出视口时仅上顶最少必要距离（底边贴窗口下缘）；与 `clampSharePop`（内容变高保持底边不越界）一致。面板因此始终从按钮下方展开：正常/临界窗口完全不遮画面，仅极小窗口贴底上顶最少距离。

**验收增量（Task 11 验收表追加；24）**：

| # | 操作 | 预期 |
|---|------|------|
| 24 | 预览视图点右下 ⚙（含窗口拉矮到装不下面板） | 面板始终从按钮下方展开（底边不越出窗口下缘），不再翻到按钮上方遮住画面 |

**执行时机**：2026-09-20 当日实现并构建（安装包 0.1.5）。

### R8（2026-09-20）：←返回后重开单路丢失另一路（自预览补齐所有活跃流）

**问题（用户实测确认）**：先开一路（投屏/摄像头）→ ←返回 → 再开另一路：视图只挂新开那一路（如只显示摄像头），不会变成「投屏主画面 + 摄像头小窗」；再返回并从自己卡片薄纱重进才恢复。先开谁后开谁都有此问题。

**根因**：四处「流启动成功 → 进自预览」入口（卡片投屏/摄像头按钮、⚙ 面板 sp-stop/sp-cam）都只调用 `videoView.preview(新流, ...)`；←返回时 `end()` 已清空视图，另一路仍在采集/广播但不再被挂进视图。

**实现**：
- `app.js`：新增 `enterSelfPreview()`——把当前所有活跃本地流逐路挂进视图（screen 先挂=主画面、camera 后挂=小窗）；上述 4 个入口统一改调它；R5 自己卡片薄纱点击同改用它（消除重复代码）。
- `video_view.js` `preview()`：同一路流已挂载时直接返回（不重建节点）——补齐调用幂等，保留小窗拖动/缩放状态。

**验收增量（Task 11 验收表追加；25）**：

| # | 操作 | 预期 |
|---|------|------|
| 25 | 开投屏 → ←返回 → 开摄像头（先摄像头后投屏亦然） | 直接变为「投屏主画面 + 摄像头小窗」，无需再返回重进；已在跑的一路不受影响 |

**执行时机**：2026-09-20 当日实现并构建（安装包 0.1.6）。

### R9（2026-09-20）：1080p 15fps 档位不生效（编码端未按档位丢帧）

**问题（用户实测确认）**：投屏档位选「1080p 15fps」后，观看端左下角统计仍显示 29~30fps。

**根因**：`configureEncoder` 里的 `framerate: spec.framerate` 在 WebCodecs 中只是码控提示，编码器不会自动丢帧——采集帧（30fps）的每一帧都被原样送入编码器，实际上行仍是 30fps。

**实现**：`video_capture.js` 帧循环新增按档位目标帧率的丢帧节流：距上次编码不足「目标周期 × 0.9」的帧直接丢弃（不画不编，同时节省 CPU）；`forceKey` 帧（首帧/换档/观众到达）不节流，保持起步低延迟；`lastEncTs` 记录上次编码时刻。摄像头档（720p30）源帧率≈目标帧率，不受影响；自然关键帧节奏按编码帧数计（15fps 下仍≈2s）。

**验收增量（Task 11 验收表追加；26）**：

| # | 操作 | 预期 |
|---|------|------|
| 26 | 投屏档位切「1080p 15fps」 | 预览/观看端左下角显示约 15fps；切回 720p/1080p 30fps 恢复约 30fps；换档后画面正常出新 IDR、无花屏 |

**执行时机**：2026-09-20 当日实现并构建（安装包 0.1.7）。

### R10（2026-09-20）：共享提示条隐藏偶发失效（加固 + 会话日志）

**问题（用户实测确认）**：投屏时 WebView2 底部“xxx 正在共享你的屏幕”提示条的自动隐藏（28ec61f 实现）**时好时坏**；用户定位触发条件：**房间只有自己时正常隐藏，房间有其他人在时失效**。

**现状**：`indicator.rs` 匹配逻辑与房间状态无任何关联，代码自实现后未改动；两种场景的实际窗口差异需现场日志定位。

**实现（加固 + 诊断）**：
- `indicator.rs` 匹配放宽：① `Chrome_WidgetWin*` +（置顶 或 无重定向位）任一命中（原为两者必须同时满足）；② 标题含“正在共享”/“is sharing”兜底；DevTools 排除。
- 扫描范围：从“直接子进程”扩为**全部后代** msedgewebview2.exe 进程（条窗归属进程可能随 WebView2 版本变化）。
- 隐藏验证：SW_HIDE 后复查可见性，未生效写入日志标记。
- 会话日志：`%APPDATA%\com.echoroom.dev\indicator.log`（每次投屏追加一段；>256KB 自动重建；单会话上限 400 行；前 10 秒逐次全量记录，其后仅在窗口集合变化/发生隐藏/每 20 扫时记录）。

**验收增量（Task 11 验收表追加；27）**：

| # | 操作 | 预期 |
|---|------|------|
| 27 | 投屏（分别单人 / 有人观看两种场景各一次） | 提示条均被隐藏；`indicator.log` 留下两次会话的窗口枚举明细，供比对定位失效原因 |

**执行时机**：2026-09-20 当日实现并构建（安装包 0.1.8）。

### R11（2026-09-20）：聊天区链接标蓝 + 点击浏览器打开

**需求（用户朋友反馈）**：公屏消息里的链接自动标蓝，点击用系统默认浏览器打开。

**实现**：
- 前端 `app.js`：`appendMessage` 正文改由 `linkify()` 按节点拼装（纯文本 textNode + 链接 `<a class="chat-link">`，不走 innerHTML）；识别 `http(s)://` 与 `www.*`（忽略大小写）；URL 字符集限定为半角可见字符（中文全角标点自然截断），再剥尾部误吞的半角标点（成对括号如 `Foo_(bar)` 保留）；`www.` 开头显示原文、打开时补 `https://`。
- 样式 `style.css`：`.chat-link` 主题蓝 `var(--accent)` + hover 下划线 + 长链接 `word-break: break-all` 不撑破气泡。
- 后端 `bridge.rs::open_url`：`ShellExecuteW("open")` 调系统默认浏览器；仅放行 `http://`/`https://` 前缀（大小写不敏感，第二道闸），其余 scheme 拒绝。`windows` crate 新增 `Win32_UI_Shell` feature；`lib.rs` 注册 command。

**验收增量（Task 11 验收表追加；28）**：

| # | 操作 | 预期 |
|---|------|------|
| 28 | 公屏发送含链接的消息（http/https/www. 开头、中英文标点收尾、裸 `javascript:` 文本） | 链接标蓝可点；点击用系统默认浏览器打开；中文标点结尾不吞；非链接文本不可点 |

**执行时机**：2026-09-20 当日实现并构建（安装包 0.1.9）。

### R12（2026-09-20）：音量上限 200% → 400%

**需求（用户朋友反馈）**：音量拉到 200% 仍觉得偏小（对方信号弱/远场麦克风场景，2.0× 不够）。

**实现**：
- UI 滑杆上限 200 → 400：成员音量面板与投屏音量面板（`app.js` 两处 `max`）；换算不变（`gain = 值 / 100`，即 0.0–4.0 线性倍率，400% = 4.0× ≈ +12dB）。
- Rust 三处 clamp 同步 2.0 → 4.0：`set_self_gain` / `set_peer_gain` / `set_screen_gain`；`config.rs` 与 `mixer.rs` 注释同步（0.0–4.0）。
- 安全性不变：`add_scaled` 逐样本乘后 clamp 回 i16 域，混音末级 tanh 软限幅兜底，4× 不会硬削/爆音；已有配置值 ≤2.0 不受影响。

**验收增量（Task 11 验收表追加；29）**：

| # | 操作 | 预期 |
|---|------|------|
| 29 | 成员音量滑杆拉满至 400% | 听感明显大于 200%，无爆音/硬削；重开后仍为 400%（持久化） |

**执行时机**：2026-09-20 当日实现并构建（安装包 0.1.10）。
